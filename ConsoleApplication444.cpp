...
incorrect:
    ...
    if (input == 1) {
        ifstream File;
        File.open("C:\\Users\\Diana\\OneDrive\\Рабочий стол\\Прога\\mardano.txt");
        if (File.is_open()) {
            while (!File.eof()) {
                string str1;
                getline(File, str1);
...
# define N 256
void Occurrences(string str, int size, int badchar[N])
{
    int i;
    for (i = 0; i < N; i++)
        badchar[i] = -1;
    ...
    for (i = 0; i < size; i++)
        badchar[(int)str[i]] = i; // фактическое значение последнего вхождения символа
}
